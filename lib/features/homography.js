...
const {quadrilateralConvex, matrixInverse33, smallestTriangleArea, multiplyPointHomographyInhomogenous, checkThreePointsConsistent, checkFourPointsConsistent, determinant} = require('./geometry.js');
...
const computeHomography = (options) => {
  const {srcPoints, dstPoints, keyframe, debugQuerykeyframe, startDebugIndex} = options;
  ...
  if (startDebugIndex !== undefined) {
    debugIndex = startDebugIndex;
  }
  ...
  if (!_checkHeuristics({H: finalH, testPoints, keyframe, debugQuerykeyframe})) return null;
  return finalH;
}

const _checkHeuristics = ({H, testPoints, keyframe, debugQuerykeyframe}) => {
  const HInv = matrixInverse33(H, 0.00001);
  console.log("final H Inv: ", HInv, debugQuerykeyframe.HInv);
  ...
  console.log("smallest tri", smallArea, debugQuerykeyframe.smallestTriangleArea);
