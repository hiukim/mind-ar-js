...
const computeHoughMatches = (options) => {
  ...
  const numXYAngleScaleBins = numXYAngleBins *  numScaleBins;
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    querypointBinLocations.push({binX: fbinX, binY: fbinY, binAngle: fbinAngle, binScale: fbinScale});
    ...
    if (binX < 0 || binX + 1 >= numXBins || binY < 0 || binY + 1 >= numYBins || binScale < 0 || binScale +1 >= numScaleBins) continue;
...
const _mapCorrespondence = ({querypoint, keypoint, keycenterX, keycenterY, scaleOneOverLogK}) => {
  ...
  return {
    ...
    y: S[2] * keycenterX + S[2] * keycenterY + ty,
    angle: angle,
