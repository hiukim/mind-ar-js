...
const kpmExtract = (options) => {
  ...
  const {featurePoints, subPixelFeaturePoints, prunedFeaturePoints, orientedFeaturePoints} = detect({gaussianPyramid, dogPyramid});
  ...
  if (debugContent) {
    ...
    for (let i = 0; i < list2.length; i++) {
      let p1 = list1[i];
      let p2 = list2[i];
      //console.log('compare:', i, JSON.stringify(p1), JSON.stringify(p2));
  ...
  const descriptors = extract({pyramid: gaussianPyramid, points: orientedFeaturePoints});

  const refPoints = [];
  for (let i = 0; i < orientedFeaturePoints.length; i++) {
    refPoints.push({
      imageIndex: imageNo,
      x2D: orientedFeaturePoints[i].x,
      y2D: orientedFeaturePoints[i].y,
      x3D: (orientedFeaturePoints[i].x + 0.5) / dpi * 25.4, // inch to millimeter
      y3D: ((height-0.5) - orientedFeaturePoints[i].y) / dpi * 25.4, // inch to millimeter
      angle: orientedFeaturePoints[i].angle,
      scale: orientedFeaturePoints[i].sigma,
      maxima: orientedFeaturePoints[i].score > 0,
      descriptors: descriptors[i]
    })
    //console.log("ref point", i, JSON.stringify(refPoints[i]));
  }
  return refPoints;
}
