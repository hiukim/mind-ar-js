...
const extract = (options) => {
  ...
  //_debug1({filename: 'debug_' + width + '.png', width, height, data: dValue});
  ...
  _selectFeature({imageData, width, height, dpi, featureMap, templateSize: TEMPLATE_SIZE, searchSize: SEARCH_SIZE2, occSize: OCCUPANCY_SIZE, maxSimThresh: MAX_THRESH, minSimThresh: MIN_THRESH, sdThresh: SD_THRESH});
...
const _selectFeature = (options) => {
  ...
  for (let i = 0; i < image2.length; i++) {
    ...
    for (let j = 0; j < height; j++) {
      for (let i = 0; i < width; i++) {
        //console.log("debug sim: ", i, j, image2[j*width+i]);
      }
    }
