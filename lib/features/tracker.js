...
const {icpPoint} = require('../icp/icp.js');
    ...
    track: ({modelViewTransform}) => {
      ...
      for (let j = 0; j < featureSets.length; j++) {
        ...
        for (let k = 0; k < featureSets[j].coords.length; k++) {
          ...
          if (sx < 0 || sx >= tracker.targetImage.width) continue;
          if (sy < 0 || sy >= tracker.targetImage.height) continue;
      ...
      while (i < AR2_DEFAULT_SEARCH_FEATURE_NUM) {
        let k = _selectTemplate({pos, prevFeatures, candidates, num, xsize: targetImage.width, ysize: targetImage.height, randomizer: tracker.randomizer});
        ...
        const result = _tracking2dSub({targetImage, imageList, dpiList, modelViewTransform, modelViewProjectionTransform, candidate: candidates[k], debugSub: debugContent.tracking2dSub[i]});
      ...
      console.log("selected features", selectedFeatures.length, debugContent.selectedFeatures.length);
      ...
      if (selectedFeatures.length < 4) return;

      const inlierProbs = [1.0, 0.8, 0.6, 0.4, 0.0];
      let err = null;
      let newModelViewTransform = modelViewTransform;
      for (let i = 0; i < inlierProbs.length; i++) {
        let ret = _computeUpdatedTran({modelViewTransform: newModelViewTransform, selectedFeatures, projectionTransform, inlierProb: inlierProbs[i], debugContent});
        err = ret.err;
        newModelViewTransform = ret.newModelViewTransform;

        console.log("tracker icp point", i, newModelViewTransform, err, '-vs-', debugContent['getTransMat'+(i+1)], debugContent['getTransMat'+ (i+1) + 'Err']);

        //let {err, newModelViewTransform} = _computeUpdatedTran({modelViewTransform, selectedFeatures, projectionTransform, isRobustMode: true, inlierProb:0.6, debugContent})

        //console.log("tracker icp point 2", newModelViewTransform, err, '-vs-', debugContent.getTransMat2, debugContent.getTransMat2Err);
      }
...
const _computeUpdatedTran = ({modelViewTransform, projectionTransform, selectedFeatures, inlierProb, debugContent}) => {
  ...
  let ret;
  if (inlierProb < 1) {
     ret = icpPoint({initialModelViewTransform: diffModelViewTransform, projectionTransform, worldCoords, screenCoords, isRobustMode: true, inlierProb});
  } else {
     ret = icpPoint({initialModelViewTransform: diffModelViewTransform, projectionTransform, worldCoords, screenCoords, isRobustMode: false});
  }
...
const _tracking2dSub = ({targetImage, imageList, dpiList, modelViewTransform, modelViewProjectionTransform, candidate, debugSub}) => {
