  ...
  const tracker = {
    ...
    track: ({modelViewTransform}) => {
      ...
      while (i < AR2_DEFAULT_SEARCH_FEATURE_NUM) {
        ...
        curFeatures.push(result);

        num += 1;
        //if (num === 5) num = 0;
      }

      console.log("selected features", curFeatures.length, debugContent.selectedFeatures.length);
      for (let i = 0; i < curFeatures.length; i++) {
        console.log("compare", curFeatures[i].pos2D, curFeatures[i].pos3D, debugContent.selectedFeatures[i].pos2D, debugContent.selectedFeatures[i].pos3D);
      }

      // remember selected features for next frame
      tracker.prevFeatures = [];
      for (let i = 0; i < curFeatures.length; i++) {
        tracker.prevFeatures.push(curFeatures[i]);
      }
    }
  }
  return tracker;
