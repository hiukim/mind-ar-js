...
//
    ...
    for (let j = startJ; j < endJ; j++) {
      for (let i = startI; i < endI; i++) {
        ...
        const newX = originalX; // TODO
        const newY = originalY; // TODO
        const spScale = Math.min(Math.max(0, scale + u[2]), dogPyramid.numScalesPerOctaves);

        if ( Math.abs(edgeScore) < hessianThreshold && score*score >= laplacianSqrThreshold) {
...
const _solveSymmetric33 = (options) => {
  ...
  if (det < 0.0000001) return null; // determinant undefined. no solution
