...
class Detector {
  ...
  detect(inputImageT) {
    ...
    const _prunedExtremasT = this._applyPrune(extremasResultsT, dogIndexes);
  ...
  _computeLocalization(prunedExtremasT, dogIndexes, dogPyramidImagesT) {
    const nBuckets = NUM_BUCKETS_PER_DIMENSION * NUM_BUCKETS_PER_DIMENSION;
	...
	outputShape: [nBuckets, MAX_FEATURES_PER_BUCKET, 3, 3], // 3x3 pixels around the extrema
	userCode: `
	  ${dogSubCodes}
	  ...
	  void main() {
	    ...
	    int dy = coords[2]-1;
	    int dx = coords[3]-1;
	    int k = int(getExtrema(bucketIndex, featureIndex, 1));
	    int y = int(getExtrema(bucketIndex, featureIndex, 2));
	    int x = int(getExtrema(bucketIndex, featureIndex, 3));
    ...
    return tf.tidy(() => {
      ...
      const prunedExtremas = prunedExtremasT.arraySync();
	...
	for (let j = 0; j < pixels[i].length; j++) {
      ...
      for (let i = 0; i < pixels.length; i++) {
	for (let j = 0; j < pixels[i].length; j++) {
	  if (prunedExtremas[i][j][0] === 0) {
	    result[i][j] = [0, 0, 0, 0];
	    continue;
	  }

	  const k = prunedExtremas[i][j][1];
	  const dogImageT = dogImagesT[k];
	  //console.log("dog", k, dogImageT);
	  const imageHeight = dogImageT.shape[0];
	  const imageWidth = dogImageT.shape[1];
	  ...
	  const newY = prunedExtremas[i][j][2] + uy;
	  const newX = prunedExtremas[i][j][3] + ux;
	  ...
	  //let score = prunedExtremas[i][j][1] === 0? 0: pixel[1][1];
	  result[i][j] = [
	    pixel[1][1],
	    prunedExtremas[i][j][1],
	    newY,
	    newX
	  ];
	}
